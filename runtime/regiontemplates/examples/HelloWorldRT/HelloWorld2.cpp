...
void parseInputArguments(int argc, char **argv, std::string &inputFolder,
                         int *pGpu, int *pCpu, int64_t tSizes[3]){
	...
	for(int i = 0; i < argc-1; i++){
		if(argv[i][0] == '-' && argv[i][1] == 'i'){
            ...
            *pCpu = atoi(argv[1+(i++)]);
		}else if(argv[i][0] == '-' && argv[i][1] == 's' && argv[i][2] == 'g'){
            tSizes[0] = atoi(argv[1+(i++)]);
		}else if(argv[i][0] == '-' && argv[i][1] == 's' && argv[i][2] == 'a'){
            tSizes[1] = atoi(argv[1+(i++)]);
		}else if(argv[i][0] == '-' && argv[i][1] == 's' && argv[i][2] == 'c'){
            tSizes[2] = atoi(argv[1+(i++)]);
        }
	}
}
...
void printInputArguments(std::string inputFolderPath,
    ...
    std::cout << COLOR(blue) << "[SPLIT] " << OFF;
    std::cout << "Gpu tile ~ size: " << tSizes[0] << std::endl;
    std::cout << COLOR(blue) << "[SPLIT] " << OFF;
    std::cout << "Ave tile ~ size: " << tSizes[1] << std::endl;
    std::cout << COLOR(blue) << "[SPLIT] " << OFF;
    std::cout << "Cpu tile ~ size: " << tSizes[2] << std::endl;
}
...
RegionTemplateCollection* RTFromFiles(std::string inputFolderPath,
    ...
    */
    printInputArguments(inputFolderPath, pGpu, pCpu, tSizes);
	...
	for(int i = 0; i < fileList.size(); i++){
        ...
        bTiles = tiler.approxSplit(fileList[i].c_str(), pGpu, pCpu, tSizes);
...
int main (int argc, char **argv){
    ...
    int pGpu = 100, pCpu = 0;
    int64_t tSizes[3] = {1, 2, 3}; 
    std::vector<RegionTemplate *> inputRegionTemplates;
    RegionTemplateCollection *rtCollection;

    parseInputArguments(argc, argv, inputFolderPath, &pGpu, &pCpu, tSizes);
    ...
    rtCollection = RTFromFiles(inputFolderPath, pGpu, pCpu, tSizes);
